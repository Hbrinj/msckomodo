...
int view_updatememwindow(mem_window *memwindowptr) {
  GtkTreeModel *treemodel =
      gtk_tree_view_get_model(GTK_TREE_VIEW(memwindowptr->treeview_ptr));
  ...
  uchar *evaluate_window_start_pointer(mem_window * memwindowptr) {
    ...
    if (memwindowptr->addr_type ==
        MEM_WINDOW_EXPR) { /* If user expression then re-evaluate start point */
      eval_data.representation = HEXADECIMAL;
      recorded_address =
          memwindowptr->address_string; /* Temp. string pointer */
      address_entry_status =
          Evaluate(&recorded_address, &result_integer, eval_data);
  ...
  /** * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
   */
  /*Creates lists of memory addresses that are breakpoints (active and
   * inactive)*/
  ...
  boolean make_break_lists() {
    ...
    if (!trap_get_status(0, &worda,
                         &wordb)) { /* Have trap status info okay: get breaks
                                       and breaksinactive list */
      for (temp = 0; (temp < MAX_NO_OF_TRAPS) && !error; temp++)
        if (((worda >> temp) & 1) != 0) /* Breakpoint #temp defined? */
        {
          ...
          {
            //          if (((trap.misc & 0x00EF) == 0x00EF) /* All conditions
            //          except write */
            //           && ((trap.misc & 0x0F00) == 0x0F00)) flag = 1;   /*
            //           and all sizes */
            //          else                                  flag = 0;
            flag =
            ...
            if (flag) /* if needs painting */
            {
              ...
              if ((wordb >> temp) & 1)
                breaks = g_list_append(breaks, value);
              else
                breaksinactive = g_list_append(breaksinactive, value);
            }
          } else
    ...
    //  if (error) return 0;           /* May have allocated some list space
    //  anyway */
    //  else       return 1;
  }
  ...
  void add_memory_entries_to_treeview(uchar * start_address,
    ...
    void output_ordinary_data(int row, unsigned int offset, int width, int gran,
                              GList *isa, mem_win_type type) {
      int hex_entry;
      char *data, *trash;

      for (hex_entry = 0; hex_entry < (width / gran);
           hex_entry++) { /* Update hex columns */
        data = view_chararr2hexstrbe(gran, &memdata[offset + hex_entry * gran]);
        // view_update_field(treemodel, row, MIN_HEX_ENTRY + hex_entry, data);
        view_update_field(treemodel, row, COLUMN_MEM_HEX, data);
        g_free(data);
      }

      data = view_chararr2asciistr(width, &memdata[offset]);
      view_update_field(treemodel, row, COLUMN_MEM_ASCII, data);
      g_free(data);
      ...
      view_update_field(treemodel, row, COLUMN_MEM_SOURCE, data);
      g_free(data);
    ...
    int source_disassemble(source_line * src, unsigned int addr,
      ...
      if (diff < memwindowptr->width)
        increment = diff; /* Next source entry */
      else
        increment = memwindowptr->width - (addr % memwindowptr->width);
      ...
      if (increment == memwindowptr->width) /* Default window settings okay */
        output_ordinary_data(row, addr - start_addr, memwindowptr->width,
                             memwindowptr->gran, memwindowptr->isa,
                             memwindowptr->type);
      else {
        char *string, *pStr;
        int i;
        ...
        *pStr = '\0'; /* Add terminator */
        ...
        view_update_field(treemodel, row, COLUMN_MEM_HEX, string);
        view_update_field(treemodel, row, COLUMN_MEM_ASCII, ascii);
        view_update_field(treemodel, row, COLUMN_MEM_SOURCE, "");
        g_free(string);
      }
    ...
    boolean
        used_first; /* Flag to say we have already printed first source line */
    ...
    used_first == FALSE;
    address =
        g_new(char, board->memory_ptr_width); /* Allocate CB-style variable */
    ...
    for (row = 0; row < memwindowptr->count;
         row++) /* Iterate over display rows */
    {
      ...
      pWR1->address = addr;
      pWR1->pNext = NULL; /* IDs row address if clicked */
      if (memwindowptr->row_addresses == NULL)
        memwindowptr->row_addresses = pWR1;
      else
        pWR2->pNext = pWR1; /* Append */
      ...
      /* Whether ELF symbols are to be displayed */
      {
        ...
        if (misc_find_a_symbol(&name,
                               address)) /* Find symbol: value "address" */
        {                                /* If found ... */
      ...
      /* Returns a string identifying register(+) pointing to "address" (or
       * NULL) */
      ...
      for (temp = 0; temp < SPECIAL_REGISTER_COUNT; temp++) {
        ...
        if (special_registers[temp].active /* if this reg. currently exists */
            &&
            *(special_registers[temp]
                  .valid)) /*  ... and it has a valid value */
          if (!misc_chararr_sub_to_int(
                  board->memory_ptr_width, special_registers[temp].value,
                  address)) { /* If the address is same as the value of the
                                 special reg. */
            colour =
                &special_registers[temp].colour; /* set to special colour */
            if (special_registers[temp].pixmap) {
              ...
              view_update_field(treemodel, row, COLUMN_MEM_ADDR, data);
            }
      ...
      /* Address */
      if (colour_option ==
          normal) /* not a special register - (the case above) */
      {
        trash = data;
      ...
      if ((memwindowptr->type == MEM_WIN_SOURCE) &&
          (src != NULL)) { /* Might be interested in any source file records */
        if (addr == src->address) { /* Source window - fetch from record */
          int i, j, k;
          boolean corrupt;
          char *pStr;
          ...
          {            /* Highlight and disassemble */
            ...
            increment = source_disassemble(src, addr, start_addr, row,
                                           increment, ascii);
          } else {
            ...
            view_update_field(treemodel, row, COLUMN_MEM_HEX, pStr);
            view_update_field(treemodel, row, COLUMN_MEM_ASCII, ascii);
            g_free(pStr);

            view_update_field(treemodel, row, COLUMN_MEM_SOURCE, src->text);
          }
      ...
      switch (colour_option)
        ...
        case faint:     gtk_clist_set_foreground(clist, row, &view_greycolour);
                        break;
        case breakpt:   gtk_clist_set_foreground(clist, row,
      &view_orangecolour);
                        break;
...
int view_updateregwindow(reg_window *regwindowptr) {
  ...
  GtkTreeModel *treemodel;
  char *element[] = {"SPSR", "00000000",
                     "????"}; /* needed when omitting SPSR */
  reg_bank *regbank;
  ...
  void update_reg_treeview() {
    ...
    for (count = 0; count < regbank->number;
         count++) { /* All registers in current bank */
      ...
      reg_ASCII(&regbank->values[count * regbank->width], &data2[0],
                MAX_REG_SIZE);

      gtk_list_store_set(liststore, &iter, COLUMN_REG_REG, data0,
                         COLUMN_REG_HEX, data1, COLUMN_REG_ASCII, data2, -1);
  ...
  void display_or_omit_spsr_in_current(uchar * displayed_cpsr) {
    ...
    {
      if (((displayed_cpsr[0] & 0xF) == 0x0) ||
          ((displayed_cpsr[0] & 0xF) ==
           0xF)) { /* If the bottom 4 bits in CPSR is 0xF or 0x0 hide SPSR */
        if (spsr_exists) { /* if we deal with 'current' and the value of CPSR is
                              as above */
      ...
      } else /* return SPSR to 'current' reg. bank */
      {
        ...
        {                 /* add SPSR - update data fields too */
          element[1] =
              view_chararr2hexstrbe(regbank->width, /* Allocates ... */
                                    &regbank->values[SPSR * regbank->width]);
          reg_ASCII(&regbank->values[SPSR * regbank->width], &ascii[0],
                    MAX_REG_SIZE);
          element[2] = &ascii[0];
  ...
  void display_psr(int psr_ID, uchar *displayed_psr) {
    ...
    if (psr_ID == 0)
      psr_menu = CPSR_menu_handle;
    else
      psr_menu = SPSR_menu_handle;
    ...
    case 0x10:
      gtk_option_menu_set_history(GTK_OPTION_MENU(psr_menu), 0);
      break;
    case 0x11:
      gtk_option_menu_set_history(GTK_OPTION_MENU(psr_menu), 5);
      break;
    case 0x12:
      gtk_option_menu_set_history(GTK_OPTION_MENU(psr_menu), 4);
      break;
    case 0x13:
      gtk_option_menu_set_history(GTK_OPTION_MENU(psr_menu), 1);
      break;
    case 0x17:
      gtk_option_menu_set_history(GTK_OPTION_MENU(psr_menu), 2);
      break;
    case 0x1B:
      gtk_option_menu_set_history(GTK_OPTION_MENU(psr_menu), 3);
      break;
    case 0x1F:
      gtk_option_menu_set_history(GTK_OPTION_MENU(psr_menu), 6);
      break;
    default:
      gtk_option_menu_set_history(GTK_OPTION_MENU(psr_menu), 7);
      break;
    }
  ...
  if (TRACE > 5)
    g_print("view_updateregwindow\n");

  ret = 1;

  gint page = gtk_notebook_get_current_page(register_notebook);
  g_print("%d was the page\n", page);
  if (regwindowptr->regbank_no ==
      gtk_notebook_get_current_page(
          register_notebook)) { /* Only do anything if this is the front page */
    /* Only works with single register window - handle is global @@@ */
    if ((board_get_regbank(regwindowptr->regbank_no)) /* get register bank */
        || (board_mini_ping() &&
            board_get_regbank(regwindowptr->regbank_no))) { /*  successfully */
      treemodel =
          gtk_tree_view_get_model(GTK_TREE_VIEW(regwindowptr->treeview_ptr));
      ...
      if (strcmp(board->cpu_name, "ARM") ==
          0) /* Following only applies to ARM */
        display_or_omit_spsr_in_current(
            &regbank->values[CPSR * regbank->width]);

      if (all_flag_windows_have_been_created) { /* we only want to refresh those
                                                   after the widgets have been
                                                   created */
      ...
      ret = 0; /* Error in client interface */
  }

  if ((ret == 1) // @@@ BODGE This routine currently relied
      && (regwindowptr->regbank_no !=
          0)) // upon to update symbols for evaluator too
  {
...
void view_update_field(GtkTreeModel *treemodel, int row, int column,
                       char *text) {
  GtkTreePath *path;
  GtkTreeIter iter;
  GtkListStore *liststore;
  ...
  if (gtk_tree_model_get_iter(treemodel, &iter, path)) {
    ...
    gtk_list_store_insert(liststore, &iter, row);
    gtk_list_store_set(liststore, &iter, column, text, -1);
  }
