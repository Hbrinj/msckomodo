...
int view_updatememwindow(mem_window *memwindowptr) {
  ...
  void add_memory_entries_to_clist(uchar * start_address,
    ...
    void output_ordinary_data(int row, unsigned int offset, int width, int gran,
      ...
      view_update_field(clist, row, BREAKPOINT_ENTRY, "");
        ...
        if (special_registers[temp].active /* if this reg. currently exists */
          ...
          if (!misc_chararr_sub_to_int(
            ...
            if (special_registers[temp].pixmap)
              ...
              gtk_clist_set_pixtext(clist, row, 1, data, 2,
                                    special_registers[temp].pixmap,
                                    special_registers[temp].bitmap);
            else
              gtk_clist_set_text(clist, row, 1, data);
...
int view_updateregwindow(reg_window *regwindowptr) {
  ...
  void check_and_colour_reg(int row, const char* new_hex_data)
  {
      typedef enum { normal, highlight } colouring;
      char *prev_data;
      colouring col = normal;
      gtk_clist_get_text(clist, row, 1, &(prev_data));

      if(strncmp(prev_data, new_hex_data, strlen(prev_data)) != 0)
      {
          col = highlight;
      }
  ...
  void update_reg_clist() {
    ...
    for (count = 0; count < regbank->number;
      ...
      else if (regbank->values[count] != 0)
        data1 = "1";
      else
        data1 = "0";
      ...
      check_and_colour_reg(count,data1);
